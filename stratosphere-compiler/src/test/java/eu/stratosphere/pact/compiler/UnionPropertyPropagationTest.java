		...
		oPlan.accept(new Visitor<PlanNode>() {
			...
			public boolean preVisit(PlanNode visitable) {
				...
				if(visitable instanceof SingleInputPlanNode && visitable.getPactContract() instanceof PlanGroupReduceOperator){
					...
					Assert.assertTrue("Union should just forward the Partitioning to upper operator",
							inConn.getShipStrategy() == ShipStrategyType.FORWARD ); 
				 ...
				 * Must be 3 input Connections, all FlatMapOperators with a Partitioning Strategy
				 */
